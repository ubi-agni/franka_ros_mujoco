...
void init(std::string modelfile)
{
	...
	if (mjVERSION_HEADER != mj_version()) {
		ROS_ERROR_NAMED("mujoco_sim_proxy", "Headers and library have different versions");
	...
	simthread.join();
	ROS_INFO_NAMED("mujoco_sim_proxy", "Sim thread terminated");
	...
	ROS_INFO_NAMED("mujoco_sim_proxy", "Cleanup done");
}
...
{
	if (!glfwInit()) {
		ROS_ERROR_NAMED("mujoco_sim_proxy", "Could not initialize GLFW");
	...
	if (!window_) {
		glfwTerminate();
		ROS_ERROR_NAMED("mujoco_sim_proxy", "Could not create window");
...
void loadModel(void)
{
	ROS_DEBUG_NAMED("mujoco_sim_proxy", "Loading model...");
	...
	if (error[0]) {
		...
		ROS_WARN_NAMED("mujoco_sim_proxy", "Model compiled, but simulation warning (paused): \n %s\n\n", error);
