...
{
	if (except != State::MOVING and this->action_move_ != nullptr and this->action_move_->isActive()) {
	...
	if (except != State::GRASPING and this->action_grasp_ != nullptr and this->action_grasp_->isActive()) {
		franka_gripper::GraspResult result;
		result.success = static_cast<decltype(result.success)>(false);
		result.error   = message;
		this->action_grasp_->setAborted(result, result.error);
	}
	if (except != State::MOVING and this->action_homing_ != nullptr and this->action_homing_->isActive()) {
...
void FrankaGripperMujoco::onMoveGoal(const franka_gripper::MoveGoalConstPtr &goal)
{
	...
	transition(
	    State::MOVING,
	    Config{ .width_desired = goal->width, .speed_desired = goal->speed, .force_desired = 0, .tolerance = eps });
...
void FrankaGripperMujoco::onGraspGoal(const franka_gripper::GraspGoalConstPtr &goal)
{
	...
	result.success = static_cast<decltype(result.success)>(ok);
	if (not ok) {
		result.error = "When the gripper stopped (below speed of " + std::to_string(this->speed_threshold_) +
...
void FrankaGripperMujoco::onGripperActionGoal(const control_msgs::GripperCommandGoalConstPtr &goal)
{
	ROS_INFO_STREAM_NAMED("FrankaGripperMujoco",
	                      "New Gripper Command Action Goal received: " << goal->command.max_effort << "N");
	...
	transition(State::GRASPING, Config{ .width_desired = goal->command.position * 2.0 < width ? 0 : kMaxFingerWidth,
	...
	bool inside_tolerance =
	    width_d - this->tolerance_gripper_action_ < width and width < width_d + this->tolerance_gripper_action_;
