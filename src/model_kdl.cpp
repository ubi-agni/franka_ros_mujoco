    ...
    : singularity_threshold_(singularity_threshold)
{
	...
	if (not tree.getChain(root, tip, this->chain_)) {
		throw std::invalid_argument("Cannot find chain within URDF tree from root '" + root + "' to tip '" + tip +
...
void ModelKDL::augmentFrame(const std::string &name, const std::array<double, 16> &transform, KDL::Chain &chain)
{
	Eigen::Affine3d t;
	KDL::Frame f;
...
void ModelKDL::augmentFrame(const std::string &name, const std::array<double, 3> &center_of_mass, double mass,
                            const std::array<double, 9> &inertia, KDL::Chain &chain)
{
	KDL::Vector kc;
...
std::array<double, 16> ModelKDL::pose(franka::Frame frame, const std::array<double, 7> &q,
    ...
    const
{
	...
	if (error != KDL::SolverI::E_NOERROR) {
		throw std::logic_error("KDL forward kinematics pose calculation failed with error: " + strError(error));
	}
	Eigen::Affine3d p;
...
std::array<double, 42>
ModelKDL::bodyJacobian(franka::Frame frame, const std::array<double, 7> &q,
                       const std::array<double, 16> &F_T_EE, // NOLINT(readability-identifier-naming)
                       const std::array<double, 16> &EE_T_K) // NOLINT(readability-identifier-naming)
    const
{
...
std::array<double, 42>
ModelKDL::zeroJacobian(franka::Frame frame, const std::array<double, 7> &q,
                       const std::array<double, 16> &F_T_EE, // NOLINT(readability-identifier-naming)
                       const std::array<double, 16> &EE_T_K) // NOLINT(readability-identifier-naming)
    const
{
...
std::array<double, 49> ModelKDL::mass(const std::array<double, 7> &q,
...
std::array<double, 7>
ModelKDL::coriolis(const std::array<double, 7> &q, const std::array<double, 7> &dq,
                   const std::array<double, 9> &I_total, // NOLINT(readability-identifier-naming)
...
std::array<double, 7>
ModelKDL::gravity(const std::array<double, 7> &q, double m_total,
                  const std::array<double, 3> &F_x_Ctotal, // NOLINT(readability-identifier-naming)
