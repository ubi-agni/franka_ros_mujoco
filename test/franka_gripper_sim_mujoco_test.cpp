...
class GripperGraspFixtureTest : public GripperSimTestSetup,
...
protected:
	...
	{
		GripperSimTestSetup::SetUp();
		std::tie(desired_width, desired_velocity, desired_force, allowed_absolute_duration_error) = GetParam();
	}
...
class GripperGraspZeroFixtureTest
    : public GripperSimTestSetup,
      public testing::WithParamInterface<std::tuple<std::tuple<double, double, double, double>, double>>
{
protected:
	void SetUp() override
	{
...
class GripperHomingFixtureTest : public GripperSimTestSetup, public testing::WithParamInterface<double>
{
protected:
	double desired_width{ GetParam() };
};

TEST_P(GripperFailMoveFixtureTest, FailMove)
...
TEST_P(GripperMoveFixtureTest, CanPerformMove)
{ // NOLINT(cert-err58-cpp)
	auto move_goal           = franka_gripper::MoveGoal();
...
TEST_P(GripperHomingFixtureTest, CanPerformHoming)
{ // NOLINT(cert-err58-cpp)
...
TEST_P(GripperGraspFixtureTest, CanFailGraspGoesToClosedState)
{ // NOLINT(cert-err58-cpp)
...
TEST_P(GripperGraspZeroFixtureTest, CanSucceedGraspGoesToClosedState)
{ // NOLINT(cert-err58-cpp)
	double start_width = 0.08;
	auto move_goal     = franka_gripper::MoveGoal();
	move_goal.width    = start_width;
...
INSTANTIATE_TEST_CASE_P(DISABLED_GripperMoveFixtureTest, // NOLINT(cert-err58-cpp)
                        ...
                        ::testing::Values(std::make_tuple(0.06, 0.1, 0.1), std::make_tuple(0.01, 0.1, 0.1),
                                          std::make_tuple(0.07, 0.1, 0.1), std::make_tuple(0.03, 0.01, 0.6),
                                          std::make_tuple(0.07, 0.01, 0.6), std::make_tuple(0.0, 0.01, 0.6)));

INSTANTIATE_TEST_CASE_P( // NOLINT(cert-err58-cpp)
    DISABLED_GripperFailMoveFixtureTest, GripperFailMoveFixtureTest,
    ::testing::Values(std::make_tuple(0.09, 0.1, 0), std::make_tuple(std::numeric_limits<double>::quiet_NaN(), 0.1, 0),
                      std::make_tuple(-0.08, 0.1, 0), std::make_tuple(-0.0001, 0.1, 0)));

INSTANTIATE_TEST_CASE_P(DISABLED_GripperHomingFixtureTest, // NOLINT(cert-err58-cpp)
                        GripperHomingFixtureTest, ::testing::Values(0.06, 0.01, 0.03, 0.08, 0.0));

INSTANTIATE_TEST_CASE_P(DISABLED_GripperGraspFixtureTest, // NOLINT(cert-err58-cpp)
                        GripperGraspFixtureTest,
                        ::testing::Values(std::make_tuple(0.03, 0.1, 0., 0.1), std::make_tuple(0.02, 0.1, 5., 0.1),
                                          std::make_tuple(0.03, 0.01, 0., 0.6), std::make_tuple(0.07, 0.01, 5., 0.6)));

INSTANTIATE_TEST_CASE_P(
    DISABLED_GripperGraspZeroFixtureTest, // NOLINT(cert-err58-cpp)
    GripperGraspZeroFixtureTest,
    ::testing::Combine(::testing::Values(std::make_tuple(0.0, 0.1, 0., 0.1), std::make_tuple(0.0, 0.1, 5., 0.1),
                                         std::make_tuple(0.0, 0.01, 0., 0.6), std::make_tuple(0.0, 0.01, 50., 0.6),
