...
class GripperGraspFixtureTest
    : public GripperSimTestSetup,
      public testing::WithParamInterface<std::tuple<std::tuple<double, double, double, double, double, double>, double>>
{
protected:
	void SetUp() override
	{
		GripperSimTestSetup::SetUp();
		std::tie(desired_width, desired_velocity, desired_force, desired_epsilon_inner, desired_epsilon_outer,
		         allowed_absolute_duration_error) = std::get<0>(GetParam());
...
TEST_F(GripperSimTestSetup, FailMove)
...
TEST_P(GripperGraspFixtureTest, CanGrasp)
{ // NOLINT(cert-err58-cpp)
...
TEST_P(GripperFailGraspFixtureTest, CanFailGrasp)
{ // NOLINT(cert-err58-cpp)
	double start_width = 0.08;
	auto move_goal     = franka_gripper::MoveGoal();
	move_goal.width    = start_width;
...
INSTANTIATE_TEST_CASE_P(DISABLED_GripperFailGraspFixtureTest, // NOLINT(cert-err58-cpp)
