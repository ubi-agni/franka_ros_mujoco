    ...
    DISABLED_franka_hw_sim_compensates_gravity_on_F_ext)
{
	...
	for (int i = 0; i < 50; i++) {
		auto msg = ros::topic::waitForMessage<geometry_msgs::WrenchStamped>("/franka_state_controller/F_ext", n);
...
TEST_F(StateMachineFixture, statemachine_switches_from_Idle_to_UserStopped_if_user_stop_is_pressed)
{
	sm->set_current_states(sml::state<Idle>);
...
TEST_F(StateMachineFixture, statemachine_switches_from_UserStopped_to_Idle_if_user_stop_is_released)
{
	sm->set_current_states(sml::state<UserStopped>);
	sm->process_event(UserStop{ false });
...
TEST_F(StateMachineFixture, statemachine_updates_robot_mode_when_switching_from_Idle_to_UserStopped)
{
	sm->set_current_states(sml::state<Idle>);
	state.robot_mode = franka::RobotMode::kIdle;
...
TEST_F(StateMachineFixture, statemachine_updates_robot_mode_when_switching_from_Move_to_UserStopped)
{
	sm->set_current_states(sml::state<Move>);
	state.robot_mode = franka::RobotMode::kMove;
...
TEST_F(StateMachineFixture, statemachine_updates_robot_mode_when_switching_from_UserStopped_to_Idle)
{
	sm->set_current_states(sml::state<UserStopped>);
...
TEST_F(StateMachineFixture, statemachine_resets_desired_control_signals_when_switching_from_Idle_to_UserStopped)
{
	// Arange
	sm->set_current_states(sml::state<Idle>);
	for (int i = 1; i <= 7; i++) {
...
TEST_F(StateMachineFixture, statemachine_resets_desired_control_signals_when_switching_from_Move_to_UserStopped)
{
	// Arange
	sm->set_current_states(sml::state<Move>);
	for (int i = 1; i <= 7; i++) {
...
public:
	...
	virtual void SetUp()
	{
		...
		gripper   = std::make_unique<actionlib::SimpleActionClient<franka_gripper::MoveAction>>("/franka_gripper/move");
		error_recovery = std::make_unique<actionlib::SimpleActionClient<franka_msgs::ErrorRecoveryAction>>(
	...
	void openGripper()
	{
		...
		goal.speed = 0.1;
		goal.width = franka_mujoco::kMaxFingerWidth;
	...
	void errorRecovery() { error_recovery->sendGoalAndWait(franka_msgs::ErrorRecoveryGoal(), ros::Duration(5)); }
...
TEST_F(FrankaHWSimFixture, pressing_user_stop_stops_the_arm_joints)
{
	...
	auto pub = nh->advertise<geometry_msgs::PoseStamped>("/cartesian_impedance_example_controller/equilibrium_pose", 1);
	ros::Rate rate(30);
	...
	while (ros::ok()) {
		...
		auto state =
		    ros::topic::waitForMessage<franka_msgs::FrankaState>("/franka_state_controller/franka_states", *nh, timeout);
		ASSERT_TRUE(state != nullptr) << "No message on /franka_state_controller/franka_states received within "
		                              << timeout.toSec() << "s";
...
TEST_F(FrankaHWSimFixture, pressing_user_stop_lets_finger_joints_still_move)
{
	...
	while (ros::ok()) {
		...
		for (size_t i = 0; i < msg->name.size(); i++) {
			...
			EXPECT_NEAR(msg->position.at(i), franka_mujoco::kMaxFingerWidth / 2., position_tolerance);
		}
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "franka_hw_mujoco_test");
